...
class yAwareCLModel:
    ...
    def __init__(self, net, loss, loader_train, loader_val, loader_test, config, task_name, train_num, layer_control, n_iter, pretrained_path, FOLD, wandb, scheduler=None): # ADNI
        """
    ...
    def fine_tuning(self):
        ...
        if self.config.early_criteria == "AUROC" : 
            ...
            raise ValueError("--early_criteria should be either AUROC or loss")
        ###########ADDED######
        ...
        ###=====Test step (test loop)=====##
        #outGT, outPRED, test_loss, test_acc, test_auroc = self.eval_model(mode = "test")
        ####ADD code to use test_auroc too! (val_auroc쓰는 것을 보고 참조하기)
        ...
        #    ###add test_auroc here too! (valid_auroc추가하는 것 처럼 하면 됨)
        #else : #i.e. 'reg'
