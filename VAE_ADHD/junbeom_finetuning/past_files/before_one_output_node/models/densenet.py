...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(3, 12, 24, 16),
        ...
        assert mode in {'encoder', 'classifier', "CNN_flatten", "barlow_encoder","classifier_no_BN","classifier_inst_BN" }, "Unknown mode selected: %s"%mode
        ...
        elif self.mode == "barlow_encoder": #barlow encoder은 main.py에서 projector을 추가함
            #fc layer 
            self.fc = nn.Linear(num_features, num_classes)
        elif self.mode == "classifier_no_BN" : #same as barlow_encoder, just fc 가아닌 classifier
            #no final batchnorm (norm5)
            #Linear layer
            self.classifier = nn.Linear(num_features, num_classes)
        elif self.mode == "classifier_inst_BN" : 
            #do instance BN instead of regular BN because, since, 원래 ideal한 상황이라면 norm5를 이미 pretraiing에서 배워서, `runningmean, running_var` 이 accurate했을텐데, pretraiing이 아닌 downstream에서 이기 때문에 N 수가 적어서, finetuning 때 runningmean등을 배울 수 없으니 (also, shiouldn't learn batchwise) => therefor,e use instance norm because the are we take mean/var instance-wise, meaning that the small size of the sample won't affect things)
            #also, I set affine = False because we don't want only want to do lienar eval, not nonlienear eval as in yAware's mistake
            #also, I set running_tracks = False (default) because since our downstream dataset is small, we don't want to add any funny thign (i.e. make the instance BN rigid)
            self.features.add_module('inst_norm5', nn.InstanceNorm3d(num_features, affine = False))
