...
def multilabel_matrix_maker(df, binary_cols = None, multiclass_cols = None , continuous_cols = None , n_chunks = None) :
    ...
    * n_chunks : if using continouous cols are used, how many split?
    ...
    """
    df = df.copy() #copy because we don't to modify the original df 
    ...
    total_cols = []
    if binary_cols :  #this is the problematic part! (probably becuse df[col] ? 
        for col in binary_cols :
            df[col] = pd.factorize(df[col])[0] #make int o0 or 1 (factorized)
