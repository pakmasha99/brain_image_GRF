...
from yAwareContrastiveLearning_optuna_fix import yAwareCLModel
...
#=====removed because batchsize learning rate and weight decay will be handleded by optuna======#
...
def main(trial):
    ...
    #optuna lr
    args.learning_rate = trial.suggest_float("learning_rate", 1e-7,1e4, log = True)  #1e-4
    ...
    #add BN too? (instance norm or none?)
    ...
    for FOLD, (train_idx, valid_idx) in enumerate(kf.split(label_tv, label_tv[label_name])): 
        ...
        if config.mode == PRETRAINING:
            ...
            outGT, outPRED, last_epoch = model.fine_tuning() # does actual finetuning #returns validation results after early stopping is reached (or when all the epochs are reached)
...
if __name__ == "__main__":
    ...
    study.enqueue_trial(
        {'AdamW_foreach': 'True',
         'BN_option': 'inst', 
         'learning_rate': 1.050531802303288e-05, 
         'weight_decay': 1.6467213137570964e-09
        }
    )
