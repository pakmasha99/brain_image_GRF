...
from yAwareContrastiveLearning_optuna_fix_4 import yAwareCLModel
...
parser.add_argument('--lr_schedule', required = False, default = None, type = str, help= 'what lr scheduler to use : lr_plateau, cosine_annealing,onecyclelr, custom_1, custom_2, custom_3, custom_4, custom_5 ... ')
...
def main(trial):
    ...
    for FOLD, (train_idx, valid_idx) in enumerate(kf.split(label_tv, label_tv[label_name])): 
        ...
        wandb.init(project = "LR_SCHEDULE_OPTIMIZE", #"VAE_ADHD_downstream_task",
                   config = config,
                   name = "fold" + str(FOLD),
                   job_type = job_type,
            ...
            if config.task_type == 'cls':
                if config.binary_class == False :  #original mode
                    ...
                    if args.eval : 
                        ...
                        #don't want this to be done during schedule thing
    ...
    #wandb.run.summary['mean_AUC'] = mean_AUC
...
if __name__ == "__main__":
    ...
    if args.eval :
        ...
        study.optimize(main, n_trials = 3*3, timeout = 86000) #8 hrs : 28800
