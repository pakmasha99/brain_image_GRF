...
def adjust_learning_rate(args, optimizer, loader, step):
    ...
    warmup_steps = int(0.2* len(loader)) #so that we only do a little bit! (too much)
    ...
    if args.rank == 0:
        print(f"lr : {lr : .3e}, step : {step}, max_steps: {max_steps}")#, lr_w : {args.learning_rate_weights : .3e}, lr_b : {args.learning_rate_biases : .3e}")
