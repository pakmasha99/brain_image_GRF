...
class MRI_dataset(Dataset):
    ...
    def __init__(self,data_path, split, split_prop, transform, shape, MNI= True, start_time = None):
        assert len(shape) == 4, "shape must be given WITH the channel too! (C, H, W, D)"
        print("=====MRI_dataset.__init__ was called by something===")
        self.start_time = start_time #ADDED THIS
        ...
        #print(f"(in dataset) before making ImageDataset : {time.time() - self.start_time}") #does not take that much time 
        self.dataset = ImageDataset(image_files=imgs, labels = lbls, transform=basic_transform(MNI = MNI,shape=self.shape ))
        
        #print(f"(in dataset) initilaized ImageDataset : {time.time() - self.start_time}") #does not take that much time
    ...
    def __getitem__(self,idx):
        ...
        if self.split == 'train':
            ...
            if idx%100==0: #too many to do, so do only with idxes with multiples of 100
                print(f"(in dataset) time took to do (only) augmentation for idx: {idx}: {time.time() - aug_start_time}")
