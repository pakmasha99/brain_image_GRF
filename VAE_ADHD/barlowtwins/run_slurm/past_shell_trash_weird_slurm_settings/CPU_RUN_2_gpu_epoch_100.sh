...
python $base_dir/BACKUP_main_3D.py /scratch/connectome/3DCNN/data/1.ABCD/1.sMRI_fmriprep/preprocessed_masked --checkpoint-dir $base_dir/REAL_TRAINING_checkpoint_ABCD_batch_${batch_size}_2_gpu_epoch_${epochs}_lambd_${lambd}_DROP_LAST_BATCH\
 --batch-size $batch_size --print-freq 1 --save-every 1 --epochs $epochs  --lambd $lambd --workers 8 #--weight-decay $w_d #원래는 save-every 10, epochs 100
