...
#SBATCH --job-name 4_GPU_ABCD_T1_8000_SSL  #job name을 다르게 하기 위해서
#SBATCH --nodes=1
...
python $base_dir/main_3D.py /scratch/connectome/3DCNN/data/1.ABCD/1.sMRI_fmriprep/preprocessed_masked --checkpoint-dir $base_dir/checkpoint_ABCD_batch_${batch_size}_4_rtx3090_gpu_epoch_${epochs}_lambd_${lambd}\
 --batch-size $batch_size --print-freq 5 --save-every 10 --epochs $epochs  --lambd $lambd #--weight-decay $w_d #원래는 save-every 10, epochs 100
