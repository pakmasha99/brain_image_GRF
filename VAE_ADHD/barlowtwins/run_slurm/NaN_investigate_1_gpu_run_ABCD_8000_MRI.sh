...
#SBATCH -o ./NaN_test/ABCD_SSL_output_%J.out #%j : job id 가 들어가는 것
#SBATCH -e ./NaN_test/ABCD_SSL_error_%J.error
#SBATCH --ntasks=1
...
python $base_dir/solve_NaN_main_3D.py /scratch/connectome/3DCNN/data/1.ABCD/1.sMRI_fmriprep/preprocessed_masked --checkpoint-dir $base_dir/checkpoint_ABCD_atest_batch_${batch_size}_1_rtx3090_gpu_epoch_${epochs}\
 --batch-size $batch_size --print-freq 1 --save-every 50 --epochs $epochs --learning-rate-weights $lr_w --learning-rate-biases $lr_b  #--lambd $lambd #--weight-decay $w_d #원래는 save-every 10, epochs 100
