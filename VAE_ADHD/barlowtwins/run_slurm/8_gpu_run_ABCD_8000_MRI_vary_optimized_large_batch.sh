...
#SBATCH --job-name 8_GPU_ABCD_T1_8000_SSL  #job name을 다르게 하기 위해서
#SBATCH --nodes=1
#SBATCH --nodelist=node1 #used node2
#SBATCH -t 12:00:00 # Time for running job #길게 10일넘게 잡음
...
python $base_dir/main_3D.py /scratch/connectome/3DCNN/data/1.ABCD/1.sMRI_fmriprep/preprocessed_masked --checkpoint-dir $base_dir/checkpoint_ABCD_batch_${batch_size}_8_rtx3090_gpu_epoch_${epochs}_lambd_${lambd}_DROP_LAST\
 --batch-size $batch_size --print-freq 1 --save-every 10 --epochs $epochs  --lambd $lambd #--weight-decay $w_d #원래는 save-every 10, epochs 100
