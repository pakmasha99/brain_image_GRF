...
batch_size=32 #36 per gpu max
lr_w=0.002 #default 0.2
lr_b=0.000048 #default 0.0048
...
python $base_dir/main_3D.py /scratch/connectome/3DCNN/data/1.ABCD/1.sMRI_fmriprep/preprocessed_masked --checkpoint-dir $base_dir/checkpoint_ABCD_batch_${batch_size}_1_rtx3090_gpu_epoch_${epochs}_lr_w_${lr_w}_lr_b_${lr_b}\
 --batch-size $batch_size --print-freq 5 --save-every 10 --epochs $epochs --learning-rate-weights $lr_w --learning-rate-biases $lr_b  #--lambd $lambd #--weight-decay $w_d #원래는 save-every 10, epochs 100
