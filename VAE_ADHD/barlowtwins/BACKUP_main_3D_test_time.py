...
from BACKUP_dataset_test_time import MRI_dataset, transform_yAware_all

#torch.autograd.set_detect_anomaly(True) #키면 느려진다고 해서 debugging할때 말고는 끄기
#================================#
...
def main_worker(gpu, args):
    ###for testing time###
    #global start_time #set to global so that other packages can also use it 
    ...
    param_biases = []  #list of bias torch tensors (나중에 optimizer에 paramgroups로 넣음)
    ...
    dataset = MRI_dataset(data_path, "train",0.05, transform_yAware_all,(1,99, 117, 95), MNI = True, start_time = start_time)
    print(f"rank {args.rank} initilaized dataset : {time.time() - start_time}")
    #dataset = dataset_UKB.UKB_T1(data_path = data_path,split = "train", transform = dataset_UKB.Transform_yAware(MNI = True)) 
    #========================================#
    ...
    loader = torch.utils.data.DataLoader(
        ...
        pin_memory=True, sampler=sampler, drop_last = True, persistent_workers = True)
    print(f"rank {args.rank} made loader  : {time.time()- start_time}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print(f"rank {args.rank} begin saving, iteration finished : {time.time()- start_time}")
        if args.rank == 0:
            ...
            #==========================================#
        print(f"rank {args.rank} end of epoch : {time.time()- start_time}")
    if args.rank == 0:
...
def adjust_learning_rate(args, optimizer, loader, step):
    ...
    optimizer.param_groups[0]['lr'] = lr * args.learning_rate_weights #i.e. 여기서 bias의 lr값을 설정함 (manually)
    optimizer.param_groups[1]['lr'] = lr * args.learning_rate_biases #i.e. 여기서 weight의 lr을 설정함 (manually)
