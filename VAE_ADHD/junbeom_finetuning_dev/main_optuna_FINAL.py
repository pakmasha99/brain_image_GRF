...
from yAwareContrastiveLearning_optuna_FINAL import yAwareCLModel
...
from finetune_utils import get_dict, test_data_analysis_2, ensemble_prediction,  get_info_fold, BCEWithLogitsLoss_smoothing #get dict to create dataset
...
#from sk_multilearn_PR_reproducible.skmultilearn.model_selection import IterativeStratification  #TODO IF PERFORMANCE DOES NOT CHANGE, THIS MAY BE THE PROBLEM!
...
parser.add_argument("--mode", type=str, choices=["pretraining", "finetuning"], required=False, default="finetuning", 
...
parser.add_argument("--stratify", type=str, choices=["KFold","strat", "balan", "iter_strat", "balan_iter_strat", "balan_strat"], required=False, # ADNI
...
parser.add_argument('--input_option', required = True, type = str, help='possible options : yAware, talairch_half, BT_org, or BT_unet,  which option top use when putting in input (reshape? padd or crop? at which dimensions?')
...
parser.add_argument('--run_where', required = True, default = 'sdcc', type = str, help= 'where to run the thing (decides whether to run config.py or config_lab.py, options : sdcc or lab or KISTI')
...
parser.add_argument('--lr_schedule', required = False, default = None, type = str, help= 'what lr scheduler to use : lr_plateau, cosine_annealing_decay, cosine_annealing,onecyclelr, custom_1, custom_2, custom_3, custom_4,5,6, cosine_annealing_faster, SGDR_1, SGDR_2, SGDR_3 (SGD+momentum+COSINEANNEALINGWARMRESTARTS), ... ')

parser.add_argument('--nb_epochs', required = False, type = int, default = None, help ="custom set epoch if wanted." )
parser.add_argument('--early_criteria' , required = False, default = 'loss', type = str , help = "use valid AUROC or loss or none for early stopping criteria?(options : 'AUROC' or 'loss' or 'none' ")
...
parser.add_argument('--AMP', required = False, default = True, type = str2bool, help = "if True, uses AMP, if False does not")
parser.add_argument('--workers', required = False, default = 8, type = int, help = "number of workers to use (default of 8)")
parser.add_argument('--wandb_name',required = False, default = "default_WANDB_file", type = str, help = "wandb where to save" )


parser.add_argument('--patience',required = False, default = 40, type = int, help = "how much patience?" ) #patience 40으로 되도록

parser.add_argument('--prune', required = False, default = False, type = str2bool, help = "whether to prune based on AUROC or not (only prunes based on first fold)")

parser.add_argument('--upweight', required = False, default = False, type = str2bool, help = "whether or not to do pruning")

###BN 은 나중에 필요성이 느껴지면 implement 하기!
parser.add_argument('--BN' , required = False, default = "default",type = str , help = "which BN to use")
parser.add_argument('--smoothing' , required = False, default = True, type = str2bool , help = "whether to use label smoothing or not")

print("config에서 workers 빼기! (밑에서 보듯이 workers를 이제 cmd상에서 지정해주니)")
...
from config_NEW import Config, PRETRAINING, FINE_TUNING
mode = PRETRAINING if args.mode == "pretraining" else FINE_TUNING
config = Config(mode, args.task, args.run_where)


if args.stratify in ['balan', "balan_iter_strat", "balan_strat"] and args.upweight == True :  #i.e. when two imbalanced thing are activate together
    raise ValueError("Only raise either balan or upweight, not both!!")
...
def main(trial):
    ...
    if args.lr_range :  #if lr range was specified
        ...
        args.learning_rate = trial.suggest_float("learning_rate", 1e-6, 1e0, log = True)  
    ...
    if args.wd_range :  #if wd range was specified
        ...
        args.weight_decay = trial.suggest_float("weight_decay", 1e-9 , 1e2, log = True) #1e-2
    config.weight_decay = args.weight_decay
    
    #optuna label smoothing #! will not do label smoothing if multiclass (not implemented) or regression (reg is obvious)
    if args.binary_class == True and not args.eval : #only do label smoothing if binary class
        if args.smoothing :
            args.label_smoothing = trial.suggest_float("label_smoothing", 0.0, 0.2, step = 0.05) #0.1
        else : 
            args.label_smoothing = 0.0
    else : #don't set label_smoothing when testing!
        args.label_smoothing = 0.0
    ...
    if args.nb_epochs : 
        ...
        config.nb_epochs = trial.suggest_int("nb_epochs", 2, 16, 2 ) 
    else : #if nb_epochs not specified and not test or test_age (i.e. we're actually running)
        config.nb_epochs = trial.suggest_int("nb_epochs", 10, 250, 30 ) #!may take too long! if so, modify it or sth!
    #TODO : 
    #4. do loss function different kernels,  
    #5. do 60,000 subjectS!!
    #!#!#! DODODODO

    #optuna BN method
    #args.BN = trial.suggest_categorical("BN_option", ['none','inst'])
    #args.BN = "inst" #inst로 고정! #NO! just use what was provided! (inst : default)
    config.BN = args.BN
    ...
    config.AMP = args.AMP
    config.num_cpu_workers = args.workers
    config.wandb_name = args.wandb_name
    config.patience = args.patience
    config.prune = args.prune
    config.upweight = args.upweight
    ...
    elif args.input_option == "talairch_half":
        config.resize_method = 'reshape'
        config.input_size = (1,80, 96, 112)
        
    elif args.input_option == "BT_org":
        config.resize_method = None
        config.input_size = (1, 99, 117, 95)
        
    elif args.input_option == "vxm_unet":
    ...
    if config.mode == PRETRAINING:
        ...
        if config.task_type == 'cls':
            ...
            if args.stratify in ['strat', "iter_strat"]:
            ...
            else: # args.stratify == 'balan' or "balan_strat", "balan_iter_strat"
                ...
                #import pdb; pdb.set_trace() #use for finding out maximum number of train_num I can use
                ...
                print("밑에서는 그 test가 아니라 _로 된다 => ... 흠. 일단 봐야할듯 (일단은 죽임)")
                train1, valid1, _ = np.split(data_1_rem.sample(frac=1, random_state=random_seed), 
            ...
            #######sanity checks#######
            if args.stratify == "KFold"  : 
                pass 
            elif args.stratify != "strat" :
                raise NotImplementedError("regression only implemented for stratify = strat or no strat")
            task_include = args.task_name.split('/')
            assert len(task_include) == 1, 'Set only one label.'
            assert config.num_classes == 1, 'Set config.num_classes == 1'
            labels = labels[labels[label_name].notna()] #해당 task데이터가 없는 아이들을 지우고 진행한다
            if any(abs(stats.zscore(labels[label_name])) > 3):
                raise ValueError(f"Values in {label_name} column have outliers Z>3, might wanna check")
            labels[label_name] = labels[label_name].astype('float') #make sure float

            #1. make sure that the .notna() filtering indeed took about the nan ones, cuz or else len(labels) below becomes weird
            test_rate = 0.2 #20% of the total data reserved for testing
            len_test = round(test_rate * len(labels))

            #train/test split => DETERMINISTICALLY (same regardless of the seed #)
            ...
            label_train, label_valid , _ = np.split(data_rem.sample(frac = 1, random_state = random_seed), 
                                                    [args.train_num , int(args.train_num*(1+config.valid_ratio))]) #_ : reamining things
    ...
    #TODO : 6만명 합치는 것! (must run more GPU!)
    ...
    label_list = []
    pred_list = []
    if config.task_type == 'reg' : #MUST BE DONE!!! (added cuz args may not have attribute stratify at all if doing reg)
        if args.stratify == "KFold"  : #i.e. no attr : use default KFold
            kf = KFold(n_splits = SPLITS)
            skf_target =  [label_tv, label_tv[label_name]]
            get_info_fold(kf.split(*skf_target), label_tv, cols_to_see)

        elif args.stratify == "strat" :
            raise NotImplementedError("THIS SHOULD BE IMPLEMENTED!! (either through disretization or sth)")
            #TODO 
        else : 
            raise NotImplementedError("regression only implemented for stratify = strat or no strat")
        #TODO... actually continuous version of iterstrat I made... so, maybe use that?? => 확인해보기!

    elif args.stratify == 'strat'or args.stratify == "balan_strat": 
    ...
    for FOLD, (train_idx, valid_idx) in enumerate(kf.split(*skf_target)):
        ...
        wandb_save_dir = os.path.join(args.save_path,args.wandb_name)
        os.makedirs(wandb_save_dir, exist_ok = True)
        wandb.init(project = args.wandb_name, #"LR_SCHEDULE_OPTIMIZE", #"VAE_ADHD_downstream_task",
                   config = config,
                   name = "fold" + str(FOLD),
                   job_type = job_type,
                   ...
                   reinit = True,
                   dir = wandb_save_dir) #becasue optuna,we have to redo the trials
        wandb.config.weight_used = str(args.pretrained_path).split('/')[-1].split('.')[0]
        
        
        config.fold = FOLD #which fold it is in
        print("FOLD" , FOLD)
        print('\n<<< {0} Fold: {1}/{2} >>>'.format(args.stratify,FOLD+1, SPLITS))
        ...
        if config.mode == PRETRAINING:
            ...
            if config.task_type == 'cls' and config.binary_class == False:
                ...
                raise NotImplementedError("not implemented yet!! (multiclass), must add things like labelsmoothing and so on")
            elif config.task_type == 'cls' and config.binary_class == True:
                if config.upweight : 
                    train_labels = np.array([y[1] for y in dataset_train])
                    pos_weight = (train_labels==0.).sum()/train_labels.sum() #i.e. the proportion of 0 to 1 (i.e. how much to boost 1 )
                    loss = BCEWithLogitsLoss_smoothing(pos_weight = torch.tensor(pos_weight), label_smoothing= config.label_smoothing)
                else : #just default version (not upweighted)
                    loss = BCEWithLogitsLoss_smoothing(label_smoothing= config.label_smoothing)
                    #EVAL, val할떄 label smoothed 된것 써도 된다! https://datascience.stackexchange.com/questions/76494/should-you-turn-off-label-smoothing-when-validating
                    #대신, AUROC 등으로 labels smoothing performance보기!
            ...
            if args.eval : 
                ...
                outGT, outPRED, loss ,stat_measures = model.eval_model(mode = 'test')
                if config.task_type == 'cls': 
                    acc = stat_measures["acc"]
                    aurocMean = stat_measures["aurocMean"]
                    model.test_loss = loss
                    model.test_acc = acc
                if config.task_type == 'reg':
                    mse, mae, rmse, r2 = stat_measures["mse"], stat_measures["mae"], stat_measures["rmse"], stat_measures["r2"]
                    model.test_loss = mse
            ...
            if config.task_type == 'cls':
                ...
                if args.eval : 
                    print(json.dumps({f"test_loss" : str(model.test_loss)}), file= model.eval_stats_file) #get what the setting was (this is actually the validaiton file and not the evaluation file, but whatever
                    mse, mae, rmse, r2 = test_data_analysis_2(config, model, outGT, outPRED, FOLD, compute_only = True)
                    mse_list.append(mse) ; mae_list.append(mae) ; rmse_list.append(rmse) ; r2_list.append(r2) ; loss_list.append(model.test_loss)
                    print(json.dumps({f"test_loss" : str(model.test_loss), f"test_MSE" : str(mse), "test_MAE" : str(mae), "test_rmse" : str(rmse), "R2" : str(r2)}), file = model.eval_stats_file)
                else : #optuna
                    mse, mae, rmse, r2 = test_data_analysis_2(config, model, outGT, outPRED, FOLD, compute_only = True)
                    mse_list.append(mse) ; mae_list.append(mae) ; rmse_list.append(rmse) ; r2_list.append(r2) ; loss_list.append(model.val_loss)
                    print(json.dumps({f"val_loss" : str(model.val_loss), f"val_MSE" : str(mse), "val_MAE" : str(mae), "val_rmse" : str(rmse), "R2" : str(r2)}), file = model.eval_stats_file)
    ...
    if args.eval : #only do if eval mode 
        with open(model.best_trial_path + "/val_stats.txt", "r") as file :
            ...
            val_weight_criteria = "mean_auroc" if config.task_type == 'cls' else "mse_list" #voting weight criteria
            val_weight_list = [d[val_weight_criteria] for d in data if val_weight_criteria in d][0] #used as weight 
            val_weight_list = [float(x) for x in val_weight_list]

        cv_val_file = open(f'{model.path2}/test_stats_final.txt', 'a', buffering=1)  #cross validation eval file 
        
        mean= ensemble_prediction(config, outGT_list, outPRED_list, stat_measure = 'mean', weights = val_weight_list, model = model, task_include = task_include)
        median= ensemble_prediction(config, outGT_list, outPRED_list, stat_measure = 'median', weights = val_weight_list, model = model, task_include = task_include)
        weighted = ensemble_prediction(config, outGT_list, outPRED_list, stat_measure = 'weighted', weights = val_weight_list, model = model, task_include = task_include)
        #hard_mode = ensemble_prediction(config, outGT_list, outPRED_list, stat_measure = 'hard_mode', weights = val_weight_list, model = model, task_include = task_include)
        #hard_weighted = ensemble_prediction(config, outGT_list, outPRED_list, stat_measure = 'hard_weighted', weights = val_weight_list, model = model, task_include = task_include)
    ...
    if config.task_type == 'cls' :
        ...
        print_CV(config, file_pth = cv_val_file, mse_list = mse_list, mae_list = mae_list, rmse_list = rmse_list, r2_list = r2_list, last_epoch_list = last_epoch_list)
    ...
    if config.task_type == 'cls' :
        ...
        #calculating validation/testing (depending on mode) mean AUC from 5 folds.
        trial_final_score = np.array(aurocMean_list).mean() #for optuna #in val mode, returns the validaiton auroc
    else : 
        trial_final_score = np.array(mse_list).mean() #for optuna #in val mode, returns the validaiton auroc

    #remove pt if not best trial
    if (study.trials_dataframe()['state'] == "COMPLETE").sum() == 0 :  #i.e. first (complete) trial
        pass 
    else : 
        if config.task_type == 'cls' : 
            if study.best_value > trial_final_score :  #i.e. not first trial, but not best trial
                [os.remove(pt_i) for pt_i in glob(model.path2+'/*.pt')]
        elif study.best_value < trial_final_score :  #i.e. task type is reg
            [os.remove(pt_i) for pt_i in glob(model.path2+'/*.pt')]

    
    return trial_final_score
...
def print_CV(config, file_pth,**kwargs):
    ...
    if config.task_type == 'cls':      
        ...
        print(json.dumps({"mse_list" : kwargs['mse_list'], "mae_list" : kwargs['mae_list'], "rmse_list" : kwargs['rmse_list'], "r2_list" : kwargs['r2_list'], "last_epoch_list" : kwargs['last_epoch_list']}), file = file_pth)
...
if __name__ == "__main__":    
    ...
    #use pruner only if using
    if args.prune:
        pruner = optuna.pruners.MedianPruner(n_startup_trials=0, n_warmup_steps=5, interval_steps=1) #wrap with patience?
    else :
        pruner = None
    
    optimize_direction = 'maximize' if config.task_type == 'cls' else 'minimize' #minimize : with reg

    study = optuna.create_study(study_name = "test_study_name", 
                                storage = storage,
                                load_if_exists = True,
                                pruner = pruner, #optuna.pruners.MedianPruner(n_startup_trials=0, n_warmup_steps=5, interval_steps=1), #wrap with patience?
                                direction = optimize_direction) #'maximize') #pruner, sampler도 나중에 넣기) 
    
    #exit the thing if already more than 150 complete done
    cancel = False
    try : 
        if (study.trials_dataframe()['state'] == "COMPLETE").sum() >= 150 : 
            print("already more than 150 trials done!")
            cancel = True if not args.eval else False
    except :
        pass
    exit() if cancel else None

    if args.eval :
        main(study.best_trial)
    else : 
        study.optimize(main, 
                       callbacks=[MaxTrialsCallback(150, states=(TrialState.COMPLETE,))],) #~2day
