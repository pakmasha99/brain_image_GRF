...
def main(trial):
    ...
    return np.array(aurocMean_list).mean() #for optuna
...
if __name__ == "__main__":
    ...
    db_dir = f"{args.save_path}/optuna_db/{args.task}-{args.layer_control}-{config.task_type}-{str(args.pretrained_path).split('/')[-1].split('.')[0]}-train_{args.train_num}"
    ...
    study.optimize(main, n_trials = 5, timeout = 60000)
