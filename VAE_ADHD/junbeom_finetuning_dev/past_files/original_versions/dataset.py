...
class ADNI_Dataset(Dataset):
    ...
    def __init__(self, config, data_csv, data_type, *args, **kwargs): # ADNI
        ...
        self.transforms.register(Normalize(), probability=1.0) #내코드는 이것을 ImageDAtaset단에서 함 
        #그리고 splititng도 ImageDAtaset단에서 함 (compared to here, where it's assumed ot already be split)

        if self.data_type == 'train' or self.config.mode == 0: #config.mode==0은 필요없다.. 무조건 finetuning이니
        ...
        ###여기가 문제! 여기서 ABCD를 loading하려고 하니 당연히 안되지 (subject list is not well made)
    ...
    def __getitem__(self, idx):
        ...
        if self.config.mode == 0: # Pre-training # consider multiple labels (list)
            raise NotImplementedError("pre-training shouldn't be used")
            #labels = []
