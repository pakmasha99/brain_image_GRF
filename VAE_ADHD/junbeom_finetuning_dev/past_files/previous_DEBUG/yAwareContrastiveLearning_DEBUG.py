###python3 NEW_main_DEBUG.py --pretrained_path ./weights/y-Aware_Contrastive_MRI_epoch_99.pth --mode finetuning --train_num 10 --layer_control freeze --stratify balan --random_seed 0 --task ADNI_ALZ_ADMCI --input__option yAware
...
class yAwareCLModel:
    ...
    def __init__(self, net, loss, loader_train, loader_val, loader_test, config, task_name, train_num, layer_control, n_iter, pretrained_path, scheduler=None): # ADNI
        ...
        ##added
        self.path2 = f"./finetune_results_yAware_seed/{config.task}-{self.layer_control}/{str(self.pretrained_path).split('/')[-1].split('.')[0]}-train_{self.train_num}/batch_{config.batch_size}-lr_{config.lr}-wd_{config.weight_decay}-tf_{config.tf}/seed_{config.random_seed}"
        try :
