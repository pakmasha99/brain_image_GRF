...
#eval 
#python3 main_optuna_FINAL.py --eval True --pretrained_path None --mode finetuning --train_num 80 --task ABCD_sex --input_option yAware --batch_size 32 --run_where lab --save_path test_reproducibiility/ABCD_80_cosineannealing_decay

###python3 main_optuna_FINAL.py --pretrained_path None --mode finetuning --train_num 50 --layer_control tune_all --stratify iter_strat --random_seed 0 --task test --input_option talairch_half --binary_class True --save_path finetune_trash --AMP True --run_where KISTI #--nb_epochs 30
#test reg
#python3 main_optuna_FINAL.py --pretrained_path None --mode finetuning --train_num 50 --stratify KFold --layer_control tune_all  --random_seed 0 --task test_age --input_option yAware --binary_class True --save_path finetune_trash --AMP True --run_where KISTI --nb_epochs 3
import os
import torch
...
class yAwareCLModel:
    ...
    def __init__(self, net, loss, loader_train, loader_val, loader_test, config, task_name, train_num, layer_control, n_iter, pretrained_path, FOLD, wandb, trial): # ADNI
        """
        ...
        if config.mode == 0: # PRETRAINING
            ...
            if layer_control == 'tune_all':
                if config.lr_schedule in ["SGDR_fast", "SGDR_medium","SGDR_slow"] : #SGD With momentum (0.9)
                    self.optimizer = torch.optim.SGD(net.parameters(), lr=config.lr, momentum=0.9, weight_decay=config.weight_decay)
                ...
                #if config.lr_schedule in ["SGDR_fast", "SGDR_medium","SGDR_slow"] : 
                if "SGDR" in config.lr_schedule : 
                    self.optimizer = torch.optim.SGD(net.classifier.parameters(), lr=config.lr, momentum = 0.9, weight_decay=config.weight_decay)#, foreach = True)
    ...
    def fine_tuning(self):
        ...
        for epoch in range(self.config.nb_epochs):
            ...
            for (inputs, labels) in self.loader:
                ...
                with torch.autocast(device_type='cuda', dtype=torch.float16, enabled = self.config.AMP):
                    y = self.model(inputs)
            ...
            outGT, outPRED, val_loss, stat_measures = self.eval_model(mode = "valid")
            if self.config.task_type == 'cls': 
                val_acc = stat_measures["acc"]
                val_auroc = stat_measures["aurocMean"]
            else : 
                val_acc = 0 #0 for compatibility
                val_auroc = 0
                mse = stat_measures["mse"]
                mae = stat_measures["mae"]
                rmse = stat_measures["rmse"]
                r2 = stat_measures["r2"]
            
            if self.config.lr_schedule : #if scheduling
                if self.config.lr_schedule == 'lr_plateau' : 
            ...
            if self.config.task_type == 'cls': # ADNI
                ...
                print("Validation MSE: {:.4f}\t Validation MAE: {:.4f}\t Validation RMSE: {:.4f}\t Validation R2: {:.4f}\t".format(val_loss, mae, rmse, r2, flush=True))
        ...
        if self.config.task_type == 'cls': # ADNI
            print("Validation accuracy: {:.2f}%\t Validation AUROC: {:.2f}\t".format(val_acc, stat_measures['aurocMean'],flush=True))
        else : 
            print("Validation MSE: {:.4f}\t Validation MAE: {:.4f}\t Validation RMSE: {:.4f}\t Validation R2: {:.4f}\t".format(
                stat_measures['mse'], stat_measures['mae'], stat_measures['rmse'], stat_measures['r2'], flush=True))
    ...
    def eval_model(self, mode):
        ...
        output : outGT(ground truth), outPRED(prediction), loss, acc (loss : normalized, acc : in terms of percentage)
        """
        ...
        with torch.no_grad():
            ...
            for (inputs, labels) in loader2use:
                ...
                elif self.config.task_type == 'cls':
                    if self.config.binary_class == False:
                        ...
                        outPRED = torch.cat((outPRED, y), 0)
                        predicted = (y > 0.0).float() #outPRED : not logit (but sigmoided) therefore the boundary 
                        acc += (predicted == labels).sum().item()
                if self.config.label_smoothing == 0.0:
                    batch_loss = self.loss(y, labels)
                elif self.config.label_smoothing != 0.0 and self.config.binary_class == True:
                    batch_loss = BCEWithLogitsLoss()(y, labels)
                else : 
                    raise NotImplementedError("label smoothing with multiclass not implemented yet")
        ...
        if self.config.task_type == 'cls':    
            ...
            aurocMean = test_data_analysis_2(self.config, self.model, outGT, outPRED, self.FOLD, compute_only = True)
            stat_measures = {"acc" : acc, "aurocMean" : aurocMean}

        else: #i.e. 'reg'
            loss = loss / len(loader2use.dataset)
            mse, mae, rmse, r2 = test_data_analysis_2(self.config, self.model, outGT, outPRED, self.FOLD, compute_only = True)
            stat_measures = {"mse" : mse, "mae" : mae, "rmse" : rmse, "r2" : r2}
        return outGT, outPRED, loss, stat_measures
                ...
                if hasattr(checkpoint, "state_dict"):
                    ...
                    if "model" in checkpoint:
                        ...
                        if len(unexpected.missing_keys) > 7 : #if still not proper,
                            raise NotImplementedError("probably improper loading was done, as more than 100 keys are missing")
                    print(f"===missing keys : {unexpected.missing_keys}=== \n===unexpected keys : {unexpected.unexpected_keys}===")
                else:
