...
#SBATCH -p volta
#SBATCH -t 24:00:00 #volta can only do four hours for voltadebug...(volta can't split gpus)
#SBATCH -N 1
...
lr_list=( 1e-2 2e-2 5e-4 1e-6 ) #( 1e-3 1e-4 1e-5 ) #default : 1e-4, but found out that that is too small for freeze
lr="${lr_list[${SLURM_ARRAY_TASK_ID}]}"

batch_list=( 8 16 ) #32 )
wd_list=( 5e-3 5e-5 5e-6 5e-7 ) #( 5e-4 5e-5 5e-6 ) #default : 5e-5
