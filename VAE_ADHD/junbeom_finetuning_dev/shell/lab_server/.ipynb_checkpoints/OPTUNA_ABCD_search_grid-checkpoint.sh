...
#SBATCH --job-name LR_searchUT80_hyper_tune_pll_yfreeze  #job name을 다르게 하기 위해서
...
sleep 1 ; python3 main_optuna_fix_5_lr_schedule.py --pretrained_path $weight --mode finetuning --train_num $num --layer_control tune_all --stratify iter_strat --random_seed 0 --task $task --input_option yAware --batch_size $batch_1 --save_path finetune_ABCD_fix_5_lr_schedule/cosine_annealing_faster --binary_class True --run_where lab --lr_schedule cosine_annealing_faster  & \
sleep 10 ; python3 main_optuna_fix_5_lr_schedule.py --pretrained_path $weight --mode finetuning --train_num $num --layer_control tune_all --stratify iter_strat --random_seed 0 --task $task --input_option yAware --batch_size $batch_1 --save_path finetune_ABCD_fix_5_lr_schedule/SGDR_1 --binary_class True --run_where lab --lr_schedule SGDR_1
