...
#SBATCH --cpus-per-task=20 #같이 하는게 훨씬 빠름(?)(test해보기.. .전에 넣은것이랑 비교해서)
...
#val_auroc로 early stopping을 해야할듯?
