...
#SBATCH --job-name balan_strict_ADHD  #job name을 다르게 하기 위해서
...
task=ABCD_ADHD_strict #( ADNI_sex ADNI_age ) #task_list=( ADNI_ALZ_ADCN ADNI_ALZ_ADMCI ADNI_ALZ_CNMCI ADNI_sex ADNI_age )
...
num=697 #best for ABCD ADHD strict balanced
...
python3 main_optuna_fix_7.py --pretrained_path $weight --mode finetuning --train_num $num --layer_control tune_all --stratify balan_iter_strat --random_seed 0 --task $task --input_option yAware --batch_size $batch_1 --save_path finetune_RESULTS/TEST_4/ADHD_strict_balan_iter_strat_undone_no_prune --binary_class True --run_where lab --early_criteria loss --lr_schedule cosine_annealing_decay --lr_range 2e-6/2e0 --wd_range 1e-2/1e1 --workers 16 --wandb_name TEST_2 --AMP True --prune ${prune} --nb_epochs 100 --patience 20
