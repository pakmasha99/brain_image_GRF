...
from finetune_utils import get_dict, test_data_analysis_2, ensemble_prediction #get dict to create dataset
...
parser.add_argument('--early_criteria' , required = False, default = 'AUROC', type = str , help = "use valid AUROC or loss for early stopping criteria?(options : 'AUROC' or 'loss or AUROC acc' ")
...
def main(trial):
    ...
    if args.layer_control == "tune_all":
        ...
        #args.learning_rate = trial.suggest_float("learning_rate", 1e-6, 1e-1, log = True)  
        #this was the case, but for time sakes will reduce to this 
    else : #freeze
    ...
    #include keys로 몇가지만 해야할듯 (Because the config also has stuff like datasets which are very large)
    #일단 여기안에있는 것들은 크게 상관없을듯.. 다 그냥 상관없는 것들이다. 
    ...
    for FOLD, (train_idx, valid_idx) in enumerate(kf.split(label_tv, label_tv[label_name])): 
        ...
        #setting up wandb
        wandb.init(project = "VAE_ADHD_downstream_task",
                   config = config,
                   name = "fold" + str(FOLD),
        ...
        if config.mode == PRETRAINING:
            ...
            model = yAwareCLModel(net, loss, loader_train, loader_val, loader_test, config, args.task_name, args.train_num, args.layer_control, None, pretrained_path, FOLD, wandb) # ADNI
            if args.eval : 
    ...
    #tensor_stack = torch.stack(outPRED_list)
    
    #soft voting 할떄 
    #avg_tensor = tensor_stack.mean(axis=0)
    #np_array = tensor_stack.cpu().numpy()
    #import pdb ; pdb.set_trace()
    #auroc_mean_arr = test_data_analysis_2(config, model, outGT_list[0], tensor_stack, task_include, 1, compute_only = True)
    #this is propbably the majority voting we are lookingfor 
    #
    if args.eval : #only do if eval mode 
        ...
        mean= ensemble_prediction(config, outGT_list, outPRED_list, stat_measure = 'mean', weights = aurocMean_list, model = model, task_include = task_include)
        median= ensemble_prediction(config, outGT_list, outPRED_list, stat_measure = 'median', weights = aurocMean_list, model = model, task_include = task_include)
        weighted = ensemble_prediction(config, outGT_list, outPRED_list, stat_measure = 'weighted', weights = aurocMean_list, model = model, task_include = task_include)
        hard_mode = ensemble_prediction(config, outGT_list, outPRED_list, stat_measure = 'hard_mode', weights = aurocMean_list, model = model, task_include = task_include)
        hard_weighted = ensemble_prediction(config, outGT_list, outPRED_list, stat_measure = 'hard_weighted', weights = aurocMean_list, model = model, task_include = task_include)
...
if __name__ == "__main__":
    ...
    #db_dir = f'{args.save_path}/{args.task}-{args.layer_control}-{config.task_type}/{str(self.pretrained_path).split('/')[-1].split('.')[0]}-train_{self.train_num}'
