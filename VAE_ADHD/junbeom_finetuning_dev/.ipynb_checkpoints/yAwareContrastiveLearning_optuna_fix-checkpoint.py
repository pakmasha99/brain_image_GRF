###python3 main_optuna_fix.py --pretrained_path ./weights/y-Aware_Contrastive_MRI_epoch_99.pth --mode finetuning --train_num 20 --layer_control tune_all --stratify strat --random_seed 0 --task test --input_option yAware --binary_class True --save_path finetune_trash --run_where sdcc #이게 그나마 잘됨 #--BN none도 없앰 (also optuna)

###python3 main_optuna_fix.py --pretrained_path ./weights/y-Aware_Contrastive_MRI_epoch_99.pth --mode finetuning --train_num 20 --layer_control tune_all --stratify strat --random_seed 0 --task ADNI_sex --input_option yAware --binary_class True --save_path finetune_test --run_where sdcc #이게 그나마 잘됨 #--BN none도 없앰 (also optuna)
...
class yAwareCLModel:
    ...
    def __init__(self, net, loss, loader_train, loader_val, loader_test, config, task_name, train_num, layer_control, n_iter, pretrained_path, FOLD, scheduler=None): # ADNI
        ...
        if config.mode == 0: # PRETRAINING
            ...
            if layer_control == 'tune_all':
                self.optimizer = torch.optim.AdamW(net.parameters(), lr=config.lr, weight_decay=config.weight_decay, foreach = config.foreach)
            elif layer_control == 'freeze':
                self.optimizer = torch.optim.AdamW(net.classifier.parameters(), lr=config.lr, weight_decay=config.weight_decay, foreach = config.foreach)
    ...
    def fine_tuning(self):
        ...
        ##ACTIVATE THIS DURING EVAL MODE
        ###밑 : 일단 안함!! (we want to give back the validation loss not hte test loss)
        ###=====Test step (test loop)=====##
