...
###python3 main_optuna_fix_4.py --pretrained_path ./weights/y-Aware_Contrastive_MRI_epoch_99.pth --mode finetuning --train_num 20 --layer_control tune_all --stratify strat --random_seed 0 --task test --input_option yAware --binary_class True --save_path finetune_trash --run_where lab
...
class yAwareCLModel:
    ...
    def __init__(self, net, loss, loader_train, loader_val, loader_test, config, task_name, train_num, layer_control, n_iter, pretrained_path, FOLD, wandb, trial): # ADNI
        ...
        if config.lr_schedule :  #if schedule exists
            ...
            elif config.lr_schedule in ["cosine_annealing", "cosine_annealing_faster"] : 
                if config.lr_schedule == "cosine_annealing_faster" : 
