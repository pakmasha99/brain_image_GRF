...
from yAwareContrastiveLearning_optuna_fix_5_dev_amp import yAwareCLModel
...
def main(trial):
    ...
    if args.wd_range :  #if wd range was specified
        ...
        args.weight_decay = trial.suggest_float("weight_decay", 1e-12 , 1e0, log = True) #1e-2
    ...
    for FOLD, (train_idx, valid_idx) in enumerate(kf.split(*skf_target)):
        ...
        wandb.init(project = "TEST_AMP", #"LR_SCHEDULE_OPTIMIZE", #"VAE_ADHD_downstream_task",
                   config = config,
                   name = "fold" + str(FOLD),
                   job_type = job_type,
...
if __name__ == "__main__":    
    ...
    if args.eval :
        ...
        study.optimize(main, n_trials = 100, timeout = 16000) #~2day
