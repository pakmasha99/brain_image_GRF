###python3 main.py --pretrained_path ./weights/y-Aware_Contrastive_MRI_epoch_99.pth --mode finetuning --train_num 10 --layer_control freeze --stratify strat --random_seed 0 --task ADNI_ALZ_ADMCI --input_option yAware --BN none
...
class yAwareCLModel:
    ...
    def __init__(self, net, loss, loader_train, loader_val, loader_test, config, task_name, train_num, layer_control, n_iter, pretrained_path, FOLD, scheduler=None): # ADNI
        ...
        ##added
        self.path2 = f"./finetune_results_FIXED_FREEZE/{config.task}-{self.layer_control}-{config.task_type}/{str(self.pretrained_path).split('/')[-1].split('.')[0]}-train_{self.train_num}/batch_{config.batch_size}-lr_{config.lr}-wd_{config.weight_decay}-tf_{config.tf}/seed_{config.random_seed}"
    ...
    def fine_tuning(self):
        ...
        with torch.no_grad():
            ...
            for (inputs, labels) in self.loader_test:
                ...
                if self.config.task_type == 'cls': # ADNI
                    #=======이 파트 : num_classes를 받아서 더 flexible하게 되도록 하기=========
...
def weight_tracker(model, module = False): #module : if model.module of just model?
    ...
    print("classifier.weight : ",torch.std(model.classifier.weight))
    print("classifier.bias   : ",torch.std(model.classifier.bias))
