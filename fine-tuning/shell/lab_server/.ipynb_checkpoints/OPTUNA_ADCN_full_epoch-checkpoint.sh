...
#SBATCH --job-name FullUT80_hyper_tune_pll_yfreeze  #job name을 다르게 하기 위해서
...
sleep 1 ; python3 main_optuna_fix_4.py --pretrained_path $weight --mode finetuning --train_num $num --layer_control tune_all --stratify strat --random_seed 0 --task $task --input_option yAware --batch_size $batch_1 --save_path finetune_ABCD_fix_3_full_epoch --binary_class True --run_where lab --lr_range 1e-6/1e-3
#sleep 10 ; python3 main_optuna_fix_3.py --pretrained_path $weight --mode finetuning --train_num $num --layer_control tune_all --stratify strat --random_seed 0 --task $task --input_option yAware --batch_size $batch_1 --save_path finetune_ABCD_fix_3_full_epoch --binary_class True --run_where lab 
